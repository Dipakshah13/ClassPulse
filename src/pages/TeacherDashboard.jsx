...
function computeStats(fb) {
  const total = fb.gotIt + fb.sortOf + fb.lost || 1;
  return {
    gotItPct:  Math.round((fb.gotIt  / total) * 100),
    sortOfPct: Math.round((fb.sortOf / total) * 100),
    lostPct:   Math.round((fb.lost   / total) * 100),
    total: fb.gotIt + fb.sortOf + fb.lost,
    clarityIndex: Math.round(((fb.gotIt * 1 + fb.sortOf * 0.5) / total) * 100),
  };
}
...
`;
  ...
  return (
    <div className="dynamic-mesh text-on-background font-body min-h-[884px] flex flex-col overflow-x-hidden relative">
      ...
      {/* ── End Session Modal ── */}
      {showEndModal && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 backdrop-blur-md p-4" onClick={() => setShowEndModal(false)}>
          <div className="glass-panel p-8 rounded-lg max-w-sm w-full mx-auto flex flex-col gap-5 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
      ...
      {/* ── Header ── */}
      <header className="fixed top-0 z-50 w-full flex flex-col sm:flex-row justify-between items-center px-4 py-3 sm:px-6 sm:py-4 bg-surface/40 backdrop-blur-3xl border-b border-outline/10 shadow-xl gap-3 sm:gap-0">
        <div className="flex flex-col items-start px-2 w-full sm:w-auto">
          <div className="flex items-center justify-between w-full sm:justify-start gap-2">
            <div className="flex items-center gap-2">
            ...
            {/* Session Code - Mobile Only Display in Logo Row */}
            <div className="sm:hidden flex items-center gap-2 bg-surface-bright/50 px-3 py-1 rounded-full border border-outline/10 backdrop-blur-md">
               <span className="text-[10px] font-black text-primary">{sessionId || '——'}</span>
            </div>
          </div>
          <span className="text-[8px] font-black uppercase tracking-[0.3em] text-on-surface-variant ml-8 -mt-0.5 block">by Dipak Shah</span>
        </div>

        <div className="flex items-center justify-end gap-2 sm:gap-4 w-auto py-1">
          {/* Always show Focus and Mute */}
          <button onClick={handleToggleFocus} className={`flex-shrink-0 flex items-center justify-center w-10 h-10 rounded-full border transition-all active:scale-95 ${focusMode ? 'bg-error text-white border-error shadow-[0_0_15px_rgba(255,110,132,0.3)]' : 'bg-white/5 border-white/10 text-on-surface-variant hover:bg-white/10'}`} title="Focus Mode">
          ...
          <button 
            ...
            className="flex-shrink-0 flex items-center justify-center w-10 h-10 rounded-full bg-white/5 border border-white/10 text-on-surface-variant hover:bg-white/10 transition-all active:scale-95"
          ...
          {/* Session Code - Desktop Only Display */}
          <div className="hidden md:flex px-4 py-1.5 rounded-full bg-surface-bright/50 backdrop-blur-md items-center gap-2 border border-outline/10">
            <span className="text-[10px] uppercase tracking-widest text-on-surface-variant font-bold leading-[1.1]">CODE</span>
          ...
          <button onClick={() => { setPinned(p => !p); showToast(pinned ? 'Unpinned' : '📌 Session pinned!', 'info'); }}
            className={`material-symbols-outlined flex-shrink-0 transition-all hover:scale-110 active:scale-95 ${pinned ? 'text-[#FACC15]' : 'text-on-surface-variant'}`}>
            push_pin
          </button>
          ...
          <div className="relative flex-shrink-0">
            ...
            {showMenu && (
              <div className="absolute right-0 top-11 z-[60] glass-panel rounded-xl overflow-hidden shadow-2xl min-w-[200px] border border-outline/10" onMouseLeave={() => setShowMenu(false)}>
                <button onClick={() => { navigate('/teacher/history'); setShowMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-on-surface-variant hover:bg-white/5 hover:text-on-surface transition-all text-left">
                  <span className="material-symbols-outlined text-base text-primary">history</span> Session Insights
                </button>
                <button onClick={() => { handleExport(); setShowMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-on-surface-variant hover:bg-white/5 hover:text-on-surface transition-all text-left border-t border-white/5">
                  <span className="material-symbols-outlined text-base text-secondary">bar_chart</span> Export Report
                </button>
                <button onClick={() => { setShowEndModal(true); setShowMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-error hover:bg-error/10 transition-all text-left border-t border-white/5">
                  <span className="material-symbols-outlined text-base">power_settings_new</span> End Session
                </button>
              </div>
            )}
          </div>
              ...
              <div className="flex-grow flex items-end justify-around gap-2 pb-4">
                {REACTION_META.map(r => (
                  <div key={r.id} className="flex flex-col items-center gap-1">
                    ...
                    <span className={`text-[11px] font-black ${r.color}`}>{reactions[r.id] || 0}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="md:col-span-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-4 overflow-y-auto max-h-[750px] pr-2 custom-scrollbar">
            {questions.length === 0 ? (
              ...
              <div key={q.id} className="glass-panel p-5 rounded-lg border-l-4 border-l-primary">
                <div className="flex justify-between mb-3"><span className="text-[9px] font-black text-on-surface-variant uppercase tracking-widest">Anonymous · {new Date(q.ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span><button onClick={() => handleUpvote(q.id)} className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded-full text-xs font-bold"><span className="material-symbols-outlined text-sm text-primary">thumb_up</span>{q.upvotes}</button></div>
                <p className="text-sm font-medium mb-4">{q.text}</p>
